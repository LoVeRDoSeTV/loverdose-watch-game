...
async function runTrackerTick() {
  ...
  try {
    ...
    if (deltaSeconds > 0 && chatterIds.length > 0) {
      ...
      const normalPoints = deltaSeconds / 3600 * 10;
      const subPoints = deltaSeconds / 3600 * 12;
      ...
      const result = await pool.query(
        ...
        RETURNING id
        `,
        [normalXp, subXp, normalPoints, subPoints, deltaSeconds, chatterIds]
      );
  ...
  async (req, res) => {
    try {
          ...
          SELECT
            ...
            ROW_NUMBER() OVER (
              ORDER BY
                ...
                CASE WHEN u.creature_id IS NOT NULL THEN u.xp ELSE NULL END DESC NULLS LAST,
                CASE WHEN u.creature_id IS NULL THEN u.watch_seconds ELSE NULL END DESC NULLS LAST,
      ...
      const leaderboard = result.rows.map(player => {
        ...
        return {
          rank: Number(player.global_rank),
          twitch_id: player.twitch_id,
          login: player.login,
          display_name: player.game_username || player.display_name,
          ...
          leaderboard_badges: Array.isArray(player.leaderboard_badges)
