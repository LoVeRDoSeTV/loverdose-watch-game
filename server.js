...
async function runTrackerTick() {
  ...
  try {
    ...
    if (!isLive) {
      ...
      return { ok: true, live: false, creditedSeconds: 0, chatters: 0, matched: 0 };
    }
  ...
  try {
    ...
    res.json({
      ...
      challenges: [
        {
          ...
          badgeImage: dofusUnlocked?.badge_image || null,
      ...
      badges: badgeResult.rows.map(item => ({
        ...
        badgeChallenge: item.badge_challenge || 'Défi à venir',
        tier: item.tier,
